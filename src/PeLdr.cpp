...
BOOL PeLdrApplyImageRelocations(DWORD dwImageBase, UINT_PTR iRelocOffset)
{
	...
	while(pBaseReloc->SizeOfBlock) {
		x = dwImageBase + pBaseReloc->VirtualAddress; // 获取需要重定位的页的地址
		dwTmp = (pBaseReloc->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / sizeof(IMAGE_RELOC); // 计算有多少个需要重定位的项
		pReloc = (PIMAGE_RELOC) (((DWORD) pBaseReloc) + sizeof(IMAGE_BASE_RELOCATION));// 获取到第一项
			...
			switch(pReloc->type) {
				case IMAGE_REL_BASED_DIR64:
					*((UINT_PTR*)(x + pReloc->offset)) += iRelocOffset;
					break;	
				case IMAGE_REL_BASED_HIGHLOW: //这个地方有错误
...
BOOL PeLdrProcessIAT(DWORD dwImageBase)
{
	...
	PIMAGE_THUNK_DATA			pThunkData;//指向IAT
	PIMAGE_THUNK_DATA			pThunkDataOrig;//指向INT
...
BOOL PeLdrNeedSelfRelocation(PE_LDR_PARAM *pe)
{
	...
	// 如果PE文件的ImageBase在程序的进程空间中，则进行重定位
...
BOOL PeLdrExecuteEP(PE_LDR_PARAM *pe)
{
	...
	// 修改PEB中PE loader中保存的当前进程的ImageBase
	_asm {
		mov eax, fs:[0x30] //TEB->PEB
		mov eax, [eax + 0xC] //PEB->Ldr
		mov pPebLdr, eax
	}
	pStart = pListEntry = (PLIST_ENTRY)(&(pPebLdr->InLoadOrderModuleList));
	pListEntry = pListEntry->Flink; // PE Loader链表中的首项对应当前
	pLdrMod = (LDR_MODULE*)(pListEntry); 
	pLdrMod->BaseAddress = (void*)pe->dwMapBase;
	DMSG("Fixing Load Module Image Base from 0x%08x to 0x%08x ", pLdrMod->BaseAddress, pe->dwMapBase);
